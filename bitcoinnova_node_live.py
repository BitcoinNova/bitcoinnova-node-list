...
REMOTE_NODES_URL = "https://raw.githubusercontent.com/BitcoinNova/bitcoinnova-nodes-json/master/bitcoinnova-nodes.json"
SLEEP_CHECK = 30  # 30 s
NODE_LIVE_LIST = []
    ...
    for node in data:
        ...
        if getinfo and getfee:
            node_list.append({
                ...
                'fee_address': getfee['address'] if 'address' in getfee and len(getfee['address']) == 95 else "",
                'fee_fee': int(getfee['amount']) if ('amount' in getfee) and (len(str(getfee['amount'])) < 10000) and (len(getfee['address']) == 95) else 0,
                'online': 1,
